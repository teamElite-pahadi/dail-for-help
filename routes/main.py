...
def service_detail(service_id):
    ...
    # Smart matching: availability + locality + rating + experience, without inventing workers.
    def match_score(w):
        locality = 35 if area and w.location and area.lower() in w.location.lower() else 0
        rating = min(30, (w.average_rating or 0) * 6)
        experience = min(20, (w.experience or 0) * 2)
        availability = 15 if w.is_available else 0
        return locality + rating + experience + availability
    workers = sorted(all_workers, key=match_score, reverse=True)
    location_match = bool(area and any(w.location and area.lower() in w.location.lower() for w in all_workers))
