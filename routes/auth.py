...
def register():
    ...
    if request.method == "POST":
        ...
        if User.query.filter_by(email=email).first():
            ...
            return render_template("auth/register.html")
        referrer = User.query.filter_by(referral_code=referral).first() if referral else None
        user = User(name=name, email=email, phone=phone, location=location, role='customer', referral_code=''.join(secrets.choice('ABCDEFGHJKLMNPQRSTUVWXYZ23456789') for _ in range(8)), referred_by=referrer.id if referrer else None)
