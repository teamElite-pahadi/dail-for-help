...
def detail(booking_id):
    ...
    if current_user.is_worker and booking.worker_id != current_user.worker_profile.id:
        return "Forbidden", 403
    from models.extra_features import BookingExtra
    extra = BookingExtra.query.filter_by(booking_id=booking.id).first()
    if extra: booking._advanced_extra = extra
    return render_template("customer/booking_detail.html", booking=booking, extra=extra)
